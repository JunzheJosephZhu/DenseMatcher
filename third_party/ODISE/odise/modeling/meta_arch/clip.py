...
class ClipAdapter(nn.Module):
    def __init__(self, name="ViT-B-32", normalize=True):
        ...
        # lightning seems to do this sequentially anyways
    ...
    def _encode_image(self, image):
        '''
        return: image_embedding - [B, 768]
                image_encoding - [B, 768, H // 14, W // 14]
        '''
    ...
    def embed_image(self, image):
        ...
        if self.normalize:
            return EmbeddedImage(F.normalize(image_embed.float(), dim=-1).to(image.dtype), image_encodings)
        else:
