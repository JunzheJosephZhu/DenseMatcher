...
def Fourier_filter(x, threshold, scale):
    ...
    mask = torch.ones((B, C, H, W), dtype=x.dtype, device=x.device) 
...
def build_ldm_from_cfg(cfg_name) -> _LatentDiffusion:
    ...
    print(time.time() - start, "downloaded config")
...
class LatentDiffusion(nn.Module):
    ...
    def __init__(
        ...
        if self.ldm.cond_stage_model is not None:
            self.ldm.cond_stage_model.__class__ = add_device_property(
                self.ldm.cond_stage_model.__class__
            )
        print(time.time() - start, "built ldm")
        start = time.time()
        self.init_checkpoint = init_checkpoint
        self.load_pretrain()
        print(time.time() - start, "loaded pretrained ldm")
    ...
    def embed_text(self, text):
        if os.path.exists("uncond_text.pt") and text == [""]:
            # print("loading embedded text")
            uncond_text = torch.load("uncond_text.pt")
        else:
            uncond_text = self.ldm.get_learned_conditioning(text)
            if text == [""]:
                if comm.get_local_rank() == 0:
                    print("saving embedded text")
                    torch.save(uncond_text, "uncond_text.pt")
        return uncond_text
...
class LdmExtractor(FeatureExtractor):
    ...
    def unet_forward(self, x, timesteps, context, cond_emb=None):
        ...
        hs = []
        t_emb = timestep_embedding(timesteps, unet.model_channels, repeat_only=False).to(x)
    ...
    def forward(self, batched_inputs):
        ...
        cond_inputs = batched_inputs.get("cond_inputs", self.ldm.embed_text(captions)).to(latent_image)
        if "caption" in batched_inputs:
...
class LdmImplicitCaptionerExtractor(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        print(time.time() - start, "instantiated extractor")
        num_timesteps=1
        start = time.time()
        self.text_embed_shape = self.ldm_extractor.ldm.embed_text([""]).shape[1:]
        print(time.time() - start, "embedded text")
        start = time.time()
        
        with torch.device("cuda") if os.environ.get("INFERENCE", False) else nullcontext():
            self.clip = ClipAdapter(name=clip_model_name, normalize=False)
        print(time.time() - start, "instantiated clip adapter")
