...
class FeatureExtractorBackbone(Backbone):
    ...
    def __init__(
        ...
        super().__init__()
        self.feature_extractor = feature_extractor # this is an LDMImplicitCaptionerExtractor
        self.use_checkpoint = use_checkpoint
        self.feature_projections = nn.ModuleList() # only used if not raw
    ...
    def ignored_state_dict(self, destination=None, prefix=""):
        ...
        for name, module in self.named_modules(remove_duplicate=True): # this recurses all children
            if module is self:
                continue
