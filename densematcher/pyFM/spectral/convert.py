...
def FM_to_p2p(FM_12, evects1, evects2, A1, use_adj=False, n_jobs=1):
    """
    ...
    emb1 = evects1[:, :k1] # [n1, k1]
    emb2 = evects2[:, :k2] @ FM_12 # [n2, k1]
    p2p_12 = knn_query(emb2, emb1,  k=1, n_jobs=n_jobs)

    emb1 = evects1[:, :k1] @ FM_12.T # [n1, k2]
    emb2 = evects2[:, :k2] # [n2, k2]
    p2p_21 = knn_query(emb1, emb2,  k=1, n_jobs=n_jobs)

    # see explanation here: https://github.com/RobinMagnet/pyFM/issues/17#issuecomment-2161404734
    
    mapped_indicator = evects2 @ FM_12 @ evects1.T @ A1
    # import matplotlib.pyplot as plt
    # plt.imsave("tmp4.png", mapped_indicator)
    return p2p_21, p2p_12, mapped_indicator  # (n2,) (n1,) (n2,n1)
...
def mesh_FM_to_p2p(FM_12, mesh1, mesh2, use_adj=False, subsample=None, n_jobs=1):
    ...
    k2, k1 = FM_12.shape
    if subsample is None:
        return FM_to_p2p(FM_12, mesh1.eigenvectors[:, :k1], mesh2.eigenvectors[:, :k2], mesh1.A,
                           use_adj=use_adj, n_jobs=n_jobs)

    else:
        sub1, sub2 = subsample
        return FM_to_p2p(FM_12, mesh1.eigenvectors[sub1, :k1], mesh2.eigenvectors[sub2, :k2], mesh1.A,
                           use_adj=use_adj, n_jobs=n_jobs)
