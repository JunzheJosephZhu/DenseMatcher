...
def get_featurizer(name, num_patches, rot_inv=False, **kwargs):
    name = name.lower()
    if name == "sd_dino":
        from .SDDINO import SDDINOFeaturizer
        patch_size = 16
        model = SDDINOFeaturizer(num_patches=num_patches, diffusion_ver='v1-5', extractor_name='dinov2_vitb14', aggre_net_weights_path=f'keypoint_2d/results_spair/best_{num_patches * patch_size}.PTH', rot_inv=rot_inv)
        dim = 768
    else:
